 ...
 *
 * The page frame number comes from the page table entry. It is the location
 * in memory where the data is located in the physical address space.
 *
 * 31            22                               0
 * +--------------+-------------------------------+
 ...
 *
 */
void pageTableLookup(const std::map<std::pair<unsigned int, unsigned int>, page_table_entry> & pageTable, unsigned int virtualAddress) {
    ...
    // 4. Check if the page table entry is valid and handle page faults accordingly.
        if (it->second.valid) {
    // 5. Calculate the physical address using the page frame number and page offset.
            unsigned int pageFrameNumber = it->second.page_frame_number;
            physicalAddress = (pageFrameNumber * PAGE_SIZE) + pageOffset;
        }
        else {
            std::cerr << "Page fault for virtual address \n";
        }
    }
    else {
        std::cerr << "Page fault for virtual address \n";
